variable "okta_org_name" {
  ...
  default = "demo-production-automation-22519"
}
...
variable "okta_api_token" {
  ...
  sensitive   = true
  default = "00rH0wZOz7ua203f8G7ab3ecdstUHeF23eZiV-la2D"
}
...
variable "oidc_app_rules" {
  description = "Policy rules for OIDC apps"
  type = map(object({
    policy_name = string
    rules = list(object({
      name        = string
